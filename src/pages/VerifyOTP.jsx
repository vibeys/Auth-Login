...
export default function VerifyOTP() {
  ...
  const [currentOTP, setCurrent] = useState(initOTP || "");
  ...
  async function handleResend() {
    ...
    try {
      ...
      setExpiry(newExpiry);
      setDigits(["", "", "", "", "", ""]);
      saveFlow({ email, otp: newOTP, expiresAt: newExpiry, step: "otp" });
      setResent(true);
      setTimeout(() => setResent(false), 4000);
      inputRefs.current[0]?.focus();
    } catch (err) {
      console.error("Resend failed:", err);
      setError(err?.text ? `Resend failed: ${err.text}` : "Failed to resend. Please try again.");
    } finally {
      setResending(false);
    }
  }
      ...
      <div className="auth-card">
        ...
        <form onSubmit={handleVerify} className="auth-form">
          ...
          <div className="otp-timer">
            {timeLeft > 0 ? (
              <>
                Code expires in&nbsp;
                <strong style={{ color: timeLeft < 60 ? "var(--error)" : "var(--accent)" }}>
                  {fmt(timeLeft)}
                </strong>
              </>
            ) : (
        ...
        <div className="resend-row">
          ...
          <button
            ...
            type="button"
          >
            {resending ? "Sending…" : canResend ? "Resend code" : `Resend in ${fmt(timeLeft - 540)}`}
          </button>
        </div>
